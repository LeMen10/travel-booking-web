...
//    }
	...
	public String showPageSearch(@RequestParam(value = "tourName", required = false) String tourName,
		...
		Page<Object[]> searchResults = toursRepository.findTours(
				tourName != null && !tourName.isEmpty() ? tourName : null, sqlStartDate, // Sử dụng biến đã chuyển đổi
				departure != null && !departure.isEmpty() ? departure : null,
				destination != null && !destination.isEmpty() ? destination : null, pageable);
	...
	public Page<Object[]> getSearchToursData(@RequestParam(value = "tourName", required = false) String tourName,
		...
		// Tạo đối tượng Pageable để thực hiện phân trang
		Pageable pageable = PageRequest.of(page, size);
		return toursRepository.findTours(tourName != null && !tourName.isEmpty() ? tourName : null, sqlStartDate, // Sử dụng biến đã chuyển đổi
				departure != null && !departure.isEmpty() ? departure : null,
				destination != null && !destination.isEmpty() ? destination : null, pageable);
	}
	/*===========================================================================================*/
